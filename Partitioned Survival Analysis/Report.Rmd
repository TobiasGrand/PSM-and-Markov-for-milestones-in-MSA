---
title: 'Partitioned Survival Model and Markov Model comparison for Multiple System
  Atrophy '
...
FitPlot <- function(IPD_data, title) {
  ...
  fit_lnorm <- survreg(Surv(IPD_data$t, IPD_data$status) ~ 1, dist = "lognormal")
...
FitPlot_trt <- function(IPD_data_trt, title) {
  ...
  fit_lnorm_trt <- survreg(Surv(IPD_data_trt$t, IPD_data_trt$status) ~ 1, dist = "lognormal")
...
# Aid-required walking: We use Weibull despite slightly better goodness-of-fit for log-normal, which is due to a little plateau at the beginning of the curve, which is not clinically plausible. It means we use Weibull through-out.

fit_weib_AidWalk <- flexsurvreg(Surv(AidWalk_time, AidWalk_status) ~ strategy_name,
  data = surv_est_data,
  dist = "weibull")

# Wheelchair: Note that we use Weibull for the same reasons although log-normal could have been used given how close BIC/AIC were between Weilbull and log-normal. 

fit_weib_Wheelchair <- flexsurvreg(Surv(Wheelchair_time, Wheelchair_status) ~ strategy_name,
  data = surv_est_data,
  dist = "weibull")

# Rest is using Weilbull
...
# psfit combines fitted event curves.

psfit <- flexsurvreg_list(
  fit_weib_AidWalk,          
  fit_weib_Wheelchair,       
  fit_weib_bedridden,
  fit_weib_death
...
# Generate PSM curves and combine into one survival model
survmods <- create_PsmCurves(psfit, input_data = surv_input_data, n = n_samples) #uncertainty not specified equal normal distribution: If "normal", then parameters are randomly drawn from their multivariate normal distribution.
